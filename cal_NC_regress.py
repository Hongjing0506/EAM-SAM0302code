...
# %%
#   plot the circulation regress onto NCR in each models
#   reg coeff.
startlevel=[-15, -8, -6]
spacinglevel=[0.75, 0.4, 0.3]
scalelevel=[0.23, 0.17, 0.14]
for num_lev,lev in enumerate([200.0, 500.0, 850.0]):
    ...
    sepl.plt_sig(
        NCRGPCP_ERA5_hgt_slope.sel(level=lev), axs[0], n, np.where(NCRGPCP_ERA5_hgt_pvalue.sel(level=lev)[::n, ::n] <= 0.05), "bright purple", 3.0,
    )
    axs[0].quiver(
        NCRGPCP_ERA5_u_slope.sel(level=lev)[::ski, ::ski],
        NCRGPCP_ERA5_v_slope.sel(level=lev)[::ski, ::ski],
    ...
    sepl.plt_sig(
        NCR_his_hgt_slope_ens.sel(level=lev), axs[1], n, np.where(NCR_his_hgt_slope_ens_mask.sel(level=lev)[::n, ::n] > 0.00), "bright purple", 3.0,
    )
    axs[1].quiver(
        NCR_his_u_slope_ens.sel(level=lev)[::ski, ::ski],
        NCR_his_v_slope_ens.sel(level=lev)[::ski, ::ski],
    ...
    for num_mod, mod in enumerate(models_array):
        con = axs[num_mod+2].contourf(
            NCR_his_hgt_slope.sel(models=mod,level=lev),
            cmap="ColdHot",
            cmap_kw={"left": 0.06, "right": 0.94, "cut": -0.1},
            levels=np.arange(startlevel[num_lev], -startlevel[num_lev]+spacinglevel[num_lev], spacinglevel[num_lev]),
            zorder=0.8,
        ...
        sepl.plt_sig(
            NCR_his_hgt_slope.sel(models=mod,level=lev), axs[num_mod+2], n, np.where(NCR_his_hgt_pvalue.sel(models=mod,level=lev)[::n, ::n] <= 0.05), "bright purple", 3.0,
        )
        axs[num_mod+2].quiver(
            NCR_his_u_slope.sel(models=mod,level=lev)[::ski, ::ski],
            NCR_his_v_slope.sel(models=mod,level=lev)[::ski, ::ski],
            zorder=1.1,
            headwidth=2.6,
            headlength=2.3,
            headaxislength=2.3,
        ...
        m = axs[num_mod+2].quiver(
            NCR_his_u_slope.sel(models=mod,level=lev).where(NCR_his_wind_mask.sel(models=mod,level=lev) > 0.0)[::ski, ::ski],
            NCR_his_v_slope.sel(models=mod,level=lev).where(NCR_his_wind_mask.sel(models=mod,level=lev) > 0.0)[::ski, ::ski],
            zorder=1.1,
            headwidth=2.6,
            headlength=2.3,
            headaxislength=2.3,
