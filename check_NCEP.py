'''
Author: ChenHJ
Date: 2022-07-11 15:42:33
LastEditors: ChenHJ
LastEditTime: 2022-07-11 16:55:21
FilePath: /chenhj/0302code/check_NCEP.py
Aim: 
Mission: 
'''
# %%
...
#   read the NCEP data to check the teleconnection
...
fhgtNCEP = xr.open_dataset("/home/ys17-23/Extension/NCEP-NCAR/monthly/hgt.mon.mean.nc")
hgtNCEP = fhgtNCEP["hgt"].sel(time=(fhgtNCEP["time"].dt.year>=1979) & (fhgtNCEP["time"].dt.year<=2014))
hgtNCEP_ver_JJA = ca.p_time(hgtNCEP, 6, 8, True)

hgtNCEP_ver_JJA.coords["time"] = preAIR_JJA.coords["time"]
(
    IndR_NCEP_hgt_slope,
    IndR_NCEP_hgt_intercept,
    IndR_NCEP_hgt_rvalue,
    IndR_NCEP_hgt_pvalue,
    IndR_NCEP_hgt_hypothesis,
) = ca.dim_linregress(preAIR_JJA, hgtNCEP_ver_JJA.sel(level=[200.0, 500.0, 850.0]))

# %% ##mark: plot the 200hPa hgt anomalies related to AIR data
pplt.rc.grid = False
pplt.rc.reso = "lo"
cl = 0  # 设置地图投影的中心纬度
...
# ======================================
con = axs[0].contourf(
    IndR_NCEP_hgt_slope.sel(level=200),
    cmap="ColdHot",
...
sepl.plt_sig(
    IndR_NCEP_hgt_slope.sel(level=200), axs[0], n, np.where(IndR_NCEP_hgt_pvalue.sel(level=200)[::n, ::n] <= 0.05), "bright purple", 3.0,
)
axs[0].format(
    rtitle="1979-2014", ltitle="AIR & NCEP",
)
# %%
